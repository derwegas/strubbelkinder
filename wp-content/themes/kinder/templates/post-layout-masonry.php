	...
	<div class="isotopePadding">
		...
		if ($show_title) {
			if (!isset($opt['links']) || $opt['links']) {
				?><h4><a href="<?php echo $post_data['post_link']; ?>"><?php echo $post_data['post_title']; ?></a></h4><?php
			} else {
		...
		?>
        <p>
		<?php echo in_array($post_data['post_format'], array('quote', 'link', 'chat')) ? $post_data['post_excerpt'] : getShortString($post_data['post_excerpt'], isset($opt['descr']) ? $opt['descr'] : get_custom_option('post_excerpt_maxlength_masonry')); ?>
		</p>
		<?php if (!isset($opt['info']) || $opt['info']) { ?>
		<div class="masonryInfo"><?php _e('Posted ', 'themerex'); ?><a href="<?php echo $post_data['post_link']; ?>" class="post_date"><?php echo $post_data['post_date']; ?></a></div>

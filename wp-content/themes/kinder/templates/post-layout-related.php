...
?>
<article class="columns1_<?php echo $opt['posts_visible']; ?> column_item_<?php echo $opt['number']; ?>">
	...
	<?php if (!isset($opt['info']) || $opt['info']) { ?>
		<div class="relatedInfo">
			...
			<a href="<?php echo $post_data['post_link']; ?>" class="post_date"><?php echo $post_data['post_date']; ?></a>
			<?php if (!empty($post_data['post_tags_links'])) { ?>
			<span class="separator">|</span> <span class="infoTags"><?php echo $post_data['post_tags_links']; ?></span>
			<?php } ?>
		</div>
